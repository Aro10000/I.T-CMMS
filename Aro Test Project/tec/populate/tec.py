...
class TecStage(models.Model):
    _inherit = "tec.task.type"
    ...
    def _populate_factories(self):
        return [
            ...
            ("description", populate.constant('tec_stage_description_{counter}')),
            ("active", populate.randomize([True, False], [0.8, 0.2])),
            ("fold", populate.randomize([True, False], [0.9, 0.1]))
        ]

class TecTec(models.Model):
    _inherit = "tec.tec"
    _populate_sizes = {"small": 10, "medium": 50, "large": 1000}
    _populate_dependencies = ["res.company", "tec.task.type"]

    def _populate_factories(self):
        company_ids = self.env.registry.populated_models["res.company"]
        stage_ids = self.env.registry.populated_models["tec.task.type"]
        ...
        return [
            ("name", populate.constant('tec_{counter}')),
            ("sequence", populate.randomize([False] + [i for i in range(1, 101)])),
            ("active", populate.randomize([True, False], [0.8, 0.2])),
...
class TecTask(models.Model):
    ...
    _populate_dependencies = ["tec.tec"]

    def _populate_factories(self):
        tec_ids = self.env.registry.populated_models["tec.tec"]
        stage_ids = self.env.registry.populated_models["tec.task.type"]
        def get_tec_id(random, **kwargs):
            return random.choice([False, False, False] + tec_ids)
        def get_stage_id(random, **kwargs):
            return random.choice([False, False] + stage_ids)
        return [
            ("name", populate.constant('tec_task_{counter}')),
            ("sequence", populate.randomize([False] + [i for i in range(1, 101)])),
            ("active", populate.randomize([True, False], [0.8, 0.2])),
            ("color", populate.randomize([False] + [i for i in range(1, 7)])),
            ("kanban_state", populate.randomize(['normal', 'done', 'blocked'])),
            ("tec_id", populate.compute(get_tec_id)),
    ...
    def _populate_set_children_tasks(self, tasks, size):
        ...
        rand = populate.Random('tec.task+children_generator')
        parents = self.env["tec.task"]
        for task in tasks:
            if not rand.getrandbits(4):
                parents |= task
        parent_ids = parents.ids
        tasks -= parents
        parent_childs = collections.defaultdict(lambda: self.env['tec.task'])
