...
class TestMultiCompanyTec(TestMultiCompanyCommon):

    @classmethod
    def setUpClass(cls):
        super(TestMultiCompanyTec, cls).setUpClass()
        ...
        cls.user_employee_company_a.write({
            'groups_id': [(4, user_group_tec_user.id)]
        })
        cls.user_manager_company_a.write({
            'groups_id': [(4, user_group_tec_manager.id)]
        })
        ...
        # create tec in both companies
        Tec = cls.env['tec.tec'].with_context({'mail_create_nolog': True, 'tracking_disable': True})
        cls.tec_company_a = Tec.create({
            'name': 'Tec Company A',
            'alias_name': 'tec+companya',
        ...
        cls.tec_company_b = Tec.create({
            'name': 'Tec Company B',
            'alias_name': 'tec+companyb',
            'partner_id': cls.partner_1.id,
            'company_id': cls.company_b.id,
            'type_ids': [
                (0, 0, {
        ...
        Task = cls.env['tec.task'].with_context({'mail_create_nolog': True, 'tracking_disable': True})
        cls.task_1 = Task.create({
            'name': 'Task 1 in Tec A',
            'user_ids': cls.user_employee_company_a,
            'tec_id': cls.tec_company_a.id
        })
        cls.task_2 = Task.create({
            'name': 'Task 2 in Tec B',
            'user_ids': cls.user_employee_company_b,
            'tec_id': cls.tec_company_b.id
        })

    def test_create_tec(self):
        """ Check tec creation in multiple companies """
        with self.sudo('manager-a'):
            tec = self.env['tec.tec'].with_context({'tracking_disable': True}).create({
                'name': 'Tec Company A',
                'partner_id': self.partner_1.id,
            })
            self.assertEqual(tec.company_id, self.env.user.company_id, "A newly created tec should be in the current user company")

            with self.switch_company(self.company_b):
                with self.assertRaises(AccessError, msg="Manager can not create tec in a company in which he is not allowed"):
                    tec = self.env['tec.tec'].with_context({'tracking_disable': True}).create({
                        'name': 'Tec Company B',
                        'partner_id': self.partner_1.id,
                ...
                # when allowed in other company, can create a tec in another company (different from the one in which you are logged)
    ...
    def test_generate_analytic_account(self):
        ...
        with self.sudo('manager-b'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                self.tec_company_a._create_analytic_account()

                self.assertEqual(self.tec_company_a.company_id, self.tec_company_a.analytic_account_id.company_id, "The analytic account created from a tec should be in the same company")

    def test_create_task(self):
        with self.sudo('employee-a'):
            # create task, set tec; the onchange will set the correct company
            with Form(self.env['tec.task'].with_context({'tracking_disable': True})) as task_form:
                task_form.name = 'Test Task in company A'
                task_form.tec_id = self.tec_company_a
            task = task_form.save()

            self.assertEqual(task.company_id, self.tec_company_a.company_id, "The company of the task should be the one from its tec.")

    def test_move_task(self):
        with self.sudo('employee-a'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                with Form(self.task_1) as task_form:
                    task_form.tec_id = self.tec_company_b
                task = task_form.save()

                self.assertEqual(task.company_id, self.company_b, "The company of the task should be the one from its tec.")
    ...
    def test_create_subtask(self):
        with self.sudo('employee-a'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                # create subtask, set parent; the onchange will set the correct company and subtask tec
                with Form(self.env['tec.task'].with_context({'tracking_disable': True, 'default_parent_id': self.task_1.id, 'default_tec_id': self.tec_company_b.id})) as task_form:
                    task_form.name = 'Test Subtask in company B'
                task = task_form.save()
                self.assertEqual(task.company_id, self.tec_company_b.company_id, "The company of the subtask should be the one from its tec, and not from its parent.")

                # set parent on existing orphan task; the onchange will set the correct company and subtask tec
                self.task_2.write({'tec_id': False})
                ...
                # `allow_subtasks` is a related to `allow_subtasks` on the tec
                # as the point of the test is to test the behavior of the task `_compute_tec_id` when there is no tec,
                ...
                view = self.env.ref('tec.view_task_form2').sudo()
                ...
                self.assertEqual(task.company_id, task.tec_id.company_id, "The company of the orphan subtask should be the one from its tec.")

    def test_cross_subtask_tec(self):
        # set up default subtask tec
        self.tec_company_a.write({'allow_subtasks': True})
        ...
        with self.sudo('employee-a'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                self.assertEqual(task.tec_id, self.task_1.tec_id, "The default tec of a subtask should be the default subtask tec of the tec from the mother task")
                self.assertEqual(task.company_id, task.tec_id.company_id, "The company of the orphan subtask should be the one from its tec.")
