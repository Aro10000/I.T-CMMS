...
class TestTectrecurrence(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super(TestTectrecurrence, cls).setUpClass()

        cls.env.user.groups_id += cls.env.ref('tec.group_tec_recurring_tasks')

        cls.stage_a = cls.env['tec.task.type'].create({'name': 'a'})
        cls.stage_b = cls.env['tec.task.type'].create({'name': 'b'})
        cls.tec_recurring = cls.env['tec.tec'].with_context({'mail_create_nolog': True}).create({
            'name': 'Recurring',
    ...
    def set_task_create_date(self, task_id, create_date):
        self.env.cr.execute("UPDATE tec_task SET create_date=%s WHERE id=%s", (create_date, task_id))

    def test_recurrence_simple(self):
        with freeze_time("2020-02-01"):
            with Form(self.env['tec.task']) as form:
                form.name = 'test recurring task'
                form.tec_id = self.tec_recurring
    ...
    def test_recurrence_cron_repeat_after(self):
        domain = [('tec_id', '=', self.tec_recurring.id)]
        with freeze_time("2020-01-01"):
            form = Form(self.env['tec.task'])
            form.name = 'test recurring task'
            ...
            self.assertEqual(self.env['tec.task'].search_count(domain), 1)
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['tec.task'].search_count(domain), 1, 'no extra task should be created')
            self.assertEqual(task.recurrence_id.recurrence_left, 2)

        with freeze_time("2020-01-15"):
            self.assertEqual(self.env['tec.task'].search_count(domain), 1)
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['tec.task'].search_count(domain), 2)
            self.assertEqual(task.recurrence_id.recurrence_left, 1)

        with freeze_time("2020-02-15"):
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['tec.task'].search_count(domain), 3)
    ...
    def test_recurrence_cron_repeat_forever(self):
        ...
        with freeze_time("2020-03-15"):
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['tec.task'].search_count(domain), 4)
    ...
    def test_recurrence_update_task(self):
        ...
        task_c, task_b, task_a = self.env['tec.task'].search([('tec_id', '=', self.tec_recurring.id)])
    ...
    def test_recurrence_next_dates_week(self):
        dates = self.env['tec.task.recurrence']._get_next_recurring_dates(
    ...
    def test_recurrence_cron_repeat_after_subtasks(self):
        ...
        self.tec_recurring.allow_subtasks = True
        parent_task = self.env['tec.task'].create({
            'name': 'Parent Task',
            'tec_id': self.tec_recurring.id
        })
        child_task = self.env['tec.task'].create({
            'name': 'Child Task',
        ...
        with freeze_time("2020-01-01"):
            with Form(child_task.with_context({'tracking_disable': True})) as form:
                form.description = 'my super recurring task bla bla bla'
                form.date_deadline = datetime(2020, 2, 1)
                form.display_tec_id = parent_task.tec_id
            ...
            tec_tasks, tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
            self.assertEqual(tec_task_count, 2)
            self.assertEqual(tec_subtask_count, 1)
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['tec.task'].search_count(domain), 2, 'no extra task should be created')
            self.assertEqual(subtask.recurrence_id.recurrence_left, 2)
            for task in tec_tasks:
                self.assertEqual(task.display_tec_id, parent_task.tec_id, "All tasks should have a display tec id set")
        ...
        with freeze_time("2020-01-15"):
            ...
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            tec_tasks, tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
            self.assertEqual(tec_task_count, 3)
            self.assertEqual(tec_subtask_count, 2)
        ...
        with freeze_time("2020-02-15"):
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            tec_tasks, tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
            self.assertEqual(tec_task_count, 4)
            self.assertEqual(tec_subtask_count, 3)
            self.assertEqual(subtask.recurrence_id.recurrence_left, 0)
            for task in tec_tasks:
                self.assertEqual(task.display_tec_id, parent_task.tec_id, "All tasks should have a display tec id set")
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            _, tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
            self.assertEqual(tec_task_count, 4)
            self.assertEqual(tec_subtask_count, 3)
            self.assertEqual(subtask.recurrence_id.recurrence_left, 0)
    ...
    def test_recurrence_cron_repeat_after_subsubtasks(self):
        ...
        domain = [('tec_id', '=', self.tec_recurring.id)]
        child_task_1, child_task_2_recurrence = self.env['tec.task'].create([
            {'name': 'Child task 1'},
        ...
        grand_child_task_1 = self.env['tec.task'].create({
            'name': 'Grandchild task 1 (recurrent)',
        })
        grand_child_task_2_recurrence = self.env['tec.task'].create({
        ...
        with freeze_time("2020-01-01"):
            ...
            grand_child_task_3, grand_child_task_4, grand_child_task_5 = self.env['tec.task'].create([
            ...
            tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
            self.assertEqual(tec_task_count, 8)
            self.assertEqual(tec_subtask_count, 7)
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            self.assertEqual(self.env['tec.task'].search_count(domain), 8, 'no extra task should be created')
            all_tasks = self.env['tec.task'].search(domain)
        ...
        for i in range(1, 5):
            with freeze_time("2020-01-%02d" % (i + 1)):
                self.env['tec.task.recurrence']._cron_create_recurring_tasks()
                tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
                self.assertEqual(tec_task_count, n + i)  # + 1 occurence of task 3
                self.assertEqual(tec_subtask_count, n + i - 1)
        with freeze_time("2020-01-11"):
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
            self.assertEqual(tec_task_count, 12)  # total = 5 occurences of task 3
            self.assertEqual(tec_subtask_count, 11)
        ...
        with freeze_time("2020-01-15"):
            ...
            self.assertEqual(tec_task_count, 12 + 1 + 4)  # 12 + the recurring task 5 + the 2 childs (3, 4) + 1 grandchild (5) + 1 grandgrandchild (6)
            self.assertEqual(tec_subtask_count, 16)
        ...
        for i in range(1, 5):
            with freeze_time("2020-01-%02d" % (i + 15)):
                self.env['tec.task.recurrence']._cron_create_recurring_tasks()
                tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
                self.assertEqual(tec_task_count, n + i)
                self.assertEqual(tec_subtask_count, n + i - 1)
        with freeze_time("2020-01-25"):
            self.env['tec.task.recurrence']._cron_create_recurring_tasks()
            tec_task_count, tec_subtask_count = get_task_and_subtask_counts(domain)
            self.assertEqual(tec_task_count, 21)
            self.assertEqual(tec_subtask_count, 20)
