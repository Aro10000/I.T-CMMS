...
class Task(models.Model):
    ...
    # Tracking of this field is done in the write function
    depend_on_ids = fields.Many2many('tec.task', relation="task_dependencies_rel", column1="task_id",
                                     column2="depends_on_id", string="Blocked By", tracking=True, copy=False,
                                     domain="[('tec_id', '!=', False), ('id', '!=', id)]")
    dependent_ids = fields.Many2many('tec.task', relation="task_dependencies_rel", column1="depends_on_id",
    ...
    def action_dependent_tasks(self):
        ...
        action = {
            'res_model': 'tec.task',
            'type': 'ir.actions.act_window',
            'context': {**self._context, 'default_depend_on_ids': [Command.link(self.id)], 'show_tec_update': False},
        }
