...
import { TecStopRecurrenceConfirmationDialog } from '../components/tec_stop_recurrence_confirmation_dialog/tec_stop_recurrence_confirmation_dialog';

class TecTaskRecurrence {
    constructor(env, dialog, orm) {
        this.env = env;
        this.dialog = dialog;
        this.orm = orm;
        this.resModel = 'tec.task';
    }
    ...
    async stopRecurrence(tasks, callback = () => {}) {
        ...
        this.dialog.add(TecStopRecurrenceConfirmationDialog, dialogProps);
    }
}
...
export const taskRecurrenceService = {
    ...
    start(env, { dialog, orm }) {
        return new TecTaskRecurrence(env, dialog, orm);
    }
};

registry.category('services').add('tec_task_recurrence', taskRecurrenceService);
