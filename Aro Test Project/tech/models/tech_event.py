...
from odoo.addons.tech.models.tech_attendee import Attendee
...
class Meeting(models.Model):
    ...
    attendee_ids = fields.One2many(
        'tech.attendee', 'event_id', 'Participant')
    ...
    def _attendees_values(self, partner_commands):
        ...
        if not self:
            attendees_to_unlink = self.env['tech.attendee']
        else:
            attendees_to_unlink = self.env['tech.attendee'].search([
    ...
    def action_open_composer(self):
        ...
        template_id = self.env['ir.model.data']._xmlid_to_res_id('tech.tech_template_meeting_update', raise_if_not_found=False)
    ...
    def _setup_alarms(self):
        ...
        for event in self:
            ...
            if any(alarm.alarm_type == 'notification' for alarm in event.alarm_ids):
                # filter events before notifying attendees through tech_alarm_manager
    ...
    def _find_attendee_batch(self):
        ...
        """
        result = defaultdict(lambda: self.env['tech.attendee'])
