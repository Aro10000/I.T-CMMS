...
class IctProjectTaskRecurrence(models.Model):
    _name = 'ict.project.task.recurrence'
    _description = 'Task Recurrence'

    task_ids = fields.One2many('ict.project.task', 'recurrence_id', copy=False)
    ...
    def _check_recurrence_days(self):
        for ict_project in self.filtered(lambda p: p.repeat_unit == 'week'):
            if not any([ict_project.mon, ict_project.tue, ict_project.wed, ict_project.thu, ict_project.fri,
                        ict_project.sat, ict_project.sun]):
                raise ValidationError(_('You should select at least one day'))
    ...
    def _create_subtasks(self, task, new_task, depth=3):
        ...
        for child in task.child_ids:
            ...
            if child.child_ids and depth > 1:
                ...
                # "grandchildren" tasks
                new_child = self.env['ict.project.task'].sudo().create(child_values)
                self._create_subtasks(child, new_child, depth=depth - 1)
            else:
                children.append(child_values)
        self.env['ict.project.task'].sudo().create(children)
    ...
    def _create_next_task(self):
        for recurrence in self:
            ...
            new_task = self.env['ict.project.task'].sudo().create(create_values)
    ...
    def _cron_create_recurring_tasks(self):
        if not self.env.user.has_group('ict_project.group_ict_project_recurring_tasks'):
            return
        today = fields.Date.today()
    ...
    def write(self, vals):
        ...
        res = super(IctProjectTaskRecurrence, self).write(vals)
