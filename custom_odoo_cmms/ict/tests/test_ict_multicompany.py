...
class TestEquipmentMulticompany(TransactionCase):
    ...
    def test_00_equipment_multicompany_user(self):
        """Test Check ict with equipment manager and user in multi company environment"""

        # Use full models
        Equipment = self.env['ict.equipment']
        ICTRequest = self.env['ict.request']
        Location = self.env['ict.equipment.category']
        ResUsers = self.env['res.users']
        ResCompany = self.env['res.company']
        ICTTeam = self.env['ict.team']

        # Use full reference.
        group_user = self.env.ref('base.group_user')
        group_manager = self.env.ref('ict.group_equipment_manager')
        ...
        # create a ict team for company A user
        team = ICTTeam.with_user(equipment_manager).create({
            'name': 'Metrology',
            'company_id': company_a.id,
        })
        # create a ict team for company B user
        teamb = ICTTeam.with_user(equipment_manager).with_context(allowed_company_ids=cids).create({
            'name': 'Subcontractor',
            'company_id': company_b.id,
        })
        ...
        # create an ict stage BY user
        with self.assertRaises(AccessError):
            self.env['ict.stage'].with_user(user).create({
                'name': 'identify corrective ict requirements',
            })

        # Create an ict request for ( User Follower ).
        ICTRequest.with_user(user).create({
        ...
        # Create an ict request for equipment_manager (Admin Follower)
        ICTRequest.with_user(equipment_manager).create({
        ...
        # Now here is total 1 ict request can be view by Normal User
        self.assertEqual(ICTRequest.with_user(equipment_manager).with_context(allowed_company_ids=cids).search_count([]), 2)
        self.assertEqual(ICTRequest.with_user(user).search_count([]), 1)
