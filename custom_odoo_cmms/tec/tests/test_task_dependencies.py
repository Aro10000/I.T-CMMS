...
class TestTaskDependencies(TestTecCommon):
    ...
    def setUpClass(cls):
        ...
        cls.task_3 = cls.env['tec.task'].with_context({'mail_create_nolog': True}).create({
    ...
    def test_task_dependencies(self):
        """ Test the task dependencies feature
            ...
            2) Checks if the task1 has the task in dependx_on_ids field.
        """
        self.assertEqual(len(self.task_1.dependx_on_ids), 0, "The task 1 should not have any dependency.")
        self.task_1.write({
            'dependx_on_ids': [Command.link(self.task_2.id)],
        })
        self.assertEqual(len(self.task_1.dependx_on_ids), 1, "The task 1 should have a dependency.")
        self.task_1.write({
            'dependx_on_ids': [Command.link(self.task_3.id)],
        })
        self.assertEqual(len(self.task_1.dependx_on_ids), 2, "The task 1 should have two dependencies.")
    ...
    def test_cyclic_dependencies(self):
        ...
        # 1) Check initial setting on three tasks
        self.assertTrue(
            len(self.task_1.dependx_on_ids) == len(self.task_2.dependx_on_ids) == len(self.task_3.dependx_on_ids) == 0,
        ...
        self.assertEqual(len(self.task_1.dependx_on_ids), 1, 'The task 1 should have one dependency.')

        # 3) Add task3 as dependency in task2
        self.task_2.write({
            'dependx_on_ids': [Command.link(self.task_3.id)],
        })
        self.assertEqual(len(self.task_2.dependx_on_ids), 1, "The task 2 should have one dependency.")

        # 4) Add task1 as dependency in task3 and check a validation error is raised
        with self.assertRaises(ValidationError), self.cr.savepoint():
            self.task_3.write({
                'dependx_on_ids': [Command.link(self.task_1.id)],
            })
        self.assertEqual(len(self.task_3.dependx_on_ids), 0, "The dependency should not be added in the task 3 because of a cyclic dependency.")
        ...
        self.assertEqual(len(self.task_2.dependx_on_ids), 1, "The number of dependencies should no change in the task 2 because of a cyclic dependency.")

    def test_tracking_dependencies(self):
        # Enable the company setting
        self.env['res.config.settings'].create({
            'group_tec_task_dependencies': True
        }).execute()
        # `dependx_on_ids` is tracked
    ...
    def test_task_dependencies_settings_change(self):
        ...
        def set_task_dependencies_setting(enabled):
            features_config = self.env["res.config.settings"].create({'group_tec_task_dependencies': enabled})
        ...
        # As the Tec General Setting group_tec_task_dependencies needs to be toggled in order
        # to be applied on the existing tecs we need to force it so that it does not depends on anything
        # (like demo data for instance)
        set_task_dependencies_setting(False)
        set_task_dependencies_setting(True)
        self.assertTrue(self.tec_pigs.allow_task_dependencies, "Tecs allow_task_dependencies should follow group_tec_task_dependencies setting changes")

        self.tec_chickens = self.env['tec.tec'].create({
            'name': 'My Chicken Tec'
        })
        self.assertTrue(self.tec_chickens.allow_task_dependencies, "New Tecs allow_task_dependencies should default to group_tec_task_dependencies")

        set_task_dependencies_setting(False)
        self.assertFalse(self.tec_pigs.allow_task_dependencies, "Tecs allow_task_dependencies should follow group_tec_task_dependencies setting changes")

        self.tec_ducks = self.env['tec.tec'].create({
            'name': 'My Ducks Tec'
        })
        self.assertFalse(self.tec_ducks.allow_task_dependencies, "New Tecs allow_task_dependencies should default to group_tec_task_dependencies")

    def test_duplicate_tec_with_task_dependencies(self):
        self.tec_pigs.allow_task_dependencies = True
        self.task_1.dependx_on_ids = self.task_2
        pigs_copy = self.tec_pigs.copy()
        ...
        self.assertEqual(task1_copy.dependx_on_ids.ids, [task2_copy.id],
                         "Copy should only create a relation between both copy if they are both part of the tec")

        task1_copy.dependx_on_ids = self.task_1

        pigs_copy_copy = pigs_copy.copy()
        task1_copy_copy = pigs_copy_copy.task_ids.filtered(lambda t: t.name == 'Pigs UserTask')

        self.assertEqual(task1_copy_copy.dependx_on_ids.ids, [self.task_1.id],
                         "Copy should not alter the relation if the other task is in a different tec")

    def test_duplicate_tec_with_subtask_dependencies(self):
        self.tec_goats.allow_task_dependencies = True
        self.tec_goats.allow_subtasks = True
        parent_task = self.env['tec.task'].with_context({'mail_create_nolog': True}).create({
            'name': 'Parent Task',
        ...
        self.assertEqual(len(single_copy_node2.dependx_on_ids), 1)
        self.assertEqual(single_copy_node2.dependx_on_ids.ids, node1.ids, 'Node1 - Node2copy relation should be present')
        ...
        self.assertEqual(len(node3.dependx_on_ids), 2)
