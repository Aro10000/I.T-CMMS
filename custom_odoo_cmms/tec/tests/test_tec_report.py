...
class TestTecReport(TestTecCommon):
    def test_avg_rating_measure(self):
        ...
        task_3 = self.env['tec.task'].create({
            'name': 'task 3',
        ...
        self.env['tec.task'].invalidate_model(['rating_avg', 'rating_last_value'])

        tasks = [self.task_1, self.task_2, task_3]
        for task in tasks:
            rating_values = task.read(['rating_avg', 'rating_last_value'])[0]
            task_report = self.env['report.tec.task.user'].search_read([('tec_id', '=', self.tec_pigs.id), ('task_id', '=', task.id)], ['rating_avg', 'rating_last_value'])[0]
