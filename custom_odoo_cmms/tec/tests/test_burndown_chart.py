...
class TestBurndownChartCommon(TestTecCommon):
    ...
    def setUpClass(cls):
        ...
        Stage = cls.env['tec.task.type']
        cls.todo_stage = Stage.create({
            'sequence': 1,
            'name': 'TODO',
            **kanban_state_vals,
        })
        cls.set_create_date('tec_task_type', cls.todo_stage.id, create_date)
        cls.in_progress_stage = Stage.create({
            'sequence': 10,
            'name': 'In Progress',
            **kanban_state_vals,
        })
        cls.set_create_date('tec_task_type', cls.in_progress_stage.id, create_date)
        cls.testing_stage = Stage.create({
            'sequence': 20,
            'name': 'Testing',
            **kanban_state_vals,
        })
        cls.set_create_date('tec_task_type', cls.testing_stage.id, create_date)
        cls.done_stage = Stage.create({
            'sequence': 30,
            'name': 'Done',
            **kanban_state_vals,
        })
        cls.set_create_date('tec_task_type', cls.done_stage.id, create_date)
        cls.stages = cls.todo_stage + cls.in_progress_stage + cls.testing_stage + cls.done_stage
        cls.tec = cls.env['tec.tec'].create({
            'name': 'Burndown Chart Test',
            'privacy_visibility': 'employees',
            'alias_name': 'tec+burndown_chart',
            'type_ids': [Command.link(stage_id) for stage_id in cls.stages.ids],
        })
        cls.set_create_date('tec_tec', cls.tec.id, create_date)
        cls.tec.invalidate_model()
        cls.milestone = cls.env['tec.milestone'].with_context({'mail_create_nolog': True}).create({
            'name': 'Test Milestone',
            'tec_id': cls.tec_pigs.id,
        })
        cls.task_a = cls.env['tec.task'].create({
            'name': 'Task A',
            'priority': 0,
            'tec_id': cls.tec.id,
            'stage_id': cls.todo_stage.id,
        })
        cls.set_create_date('tec_task', cls.task_a.id, create_date)
        cls.task_b = cls.task_a.copy({
            'name': 'Task B',
            'user_ids': [Command.set([cls.user_tecuser.id, cls.user_tecmanager.id])],
        })
        cls.set_create_date('tec_task', cls.task_b.id, create_date)
        cls.task_c = cls.task_a.copy({
            'name': 'Task C',
            'partner_id': cls.partner_1.id,
            'user_ids': [Command.link(cls.user_tecuser.id)],
        })
        cls.set_create_date('tec_task', cls.task_c.id, create_date)
        cls.task_d = cls.task_a.copy({
            'name': 'Task D',
            'milestone_id': cls.milestone.id,
            'user_ids': [Command.link(cls.user_tecmanager.id)],
        })
        cls.set_create_date('tec_task', cls.task_d.id, create_date)
        cls.task_e = cls.task_a.copy({
            'name': 'Task E',
            'partner_id': cls.partner_1.id,
        })
        cls.set_create_date('tec_task', cls.task_e.id, create_date)

        # Create a new task to check if a task without changing its stage is taken into account
        task_f = cls.env['tec.task'].create({
            'name': 'Task F',
            'priority': 0,
            'tec_id': cls.tec.id,
            'milestone_id': cls.milestone.id,
        ...
        cls.set_create_date('tec_task', task_f.id, datetime(cls.current_year - 1, 12, 20))

        cls.tec_2 = cls.env['tec.tec'].create({
            'name': 'Burndown Chart Test 2 mySearchTag',
            'privacy_visibility': 'employees',
            'alias_name': 'tec+burndown_chart+2',
            'type_ids': [Command.link(stage_id) for stage_id in cls.stages.ids],
        })
        cls.set_create_date('tec_tec', cls.tec_2.id, create_date)
        cls.tec.invalidate_model()
        cls.task_g = cls.env['tec.task'].create({
            'name': 'Task G',
            'priority': 0,
            'tec_id': cls.tec_2.id,
            'stage_id': cls.todo_stage.id,
        ...
        cls.set_create_date('tec_task', cls.task_g.id, create_date)
        cls.task_h = cls.task_g.copy({
            'name': 'Task H',
            'user_ids': [Command.link(cls.user_tecmanager.id)],
        })
        cls.set_create_date('tec_task', cls.task_h.id, create_date)
...
class TestBurndownChart(TestBurndownChartCommon):
    ...
    def check_read_group_results(self, domain, expected_results_dict):
        ...
        read_group_result = self.env['tec.task.burndown.chart.report'].read_group(
    ...
    def test_burndown_chart(self):
        burndown_chart_domain = [('display_tec_id', '!=', False)]
        tec_domain = [('tec_id', '=', self.tec.id)]

        # Check that we get the expected results for the complete data of `self.tec`.
        tec_expected_dict = {
        ...
        self.check_read_group_results(AND([burndown_chart_domain, tec_domain]), tec_expected_dict)

        # Check that we get the expected results for the complete data of `self.tec` & `self.tec_2` using an
        # `ilike` in the domain.
        all_tecs_domain_with_ilike = OR([tec_domain, [('tec_id', 'ilike', 'mySearchTag')]])
        tec_expected_dict = {key: val if key[1] != self.todo_stage.id else val + 2 for key, val in tec_expected_dict.items()}
        self.check_read_group_results(AND([burndown_chart_domain, all_tecs_domain_with_ilike]), tec_expected_dict)

        date_from, date_to = ('%s-01-01' % (self.current_year - 1), '%s-02-01' % (self.current_year - 1))
        date_and_user_domain = [('date', '>=', date_from), ('date', '<', date_to), ('user_ids', 'ilike', 'TecUser')]
        ...
        complex_domain = AND([burndown_chart_domain, all_tecs_domain_with_ilike, date_and_user_domain])
        self.check_read_group_results(complex_domain, complex_domain_expected_dict)

        date_and_user_domain = [('date', '>=', date_from), ('date', '<', date_to), ('user_ids', 'ilike', 'TecManager')]
        milestone_domain = [('milestone_id', 'ilike', 'Test')]
        complex_domain = AND([burndown_chart_domain, all_tecs_domain_with_ilike, date_and_user_domain, milestone_domain])
        complex_domain_expected_dict = {
