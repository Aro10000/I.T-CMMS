...
class TecMilestone(models.Model):
    _name = 'tec.milestone'
    _description = "Tec Milestone"
    _inherit = ['mail.thread']
    _order = 'deadline, is_reached desc, name'

    def _get_default_tec_id(self):
        return self.env.context.get('default_tec_id') or self.env.context.get('active_id')

    name = fields.Char(required=True)
    tec_id = fields.Many2one('tec.tec', required=True, default=_get_default_tec_id, ondelete='cascade')
    ...
    task_ids = fields.One2many('tec.task', 'milestone_id', 'Tasks')
    ...
    task_count = fields.Integer('# of Tasks', compute='_compute_task_count', groups='tec.group_tec_milestone')
    can_be_marked_as_done = fields.Boolean(compute='_compute_can_be_marked_as_done', groups='tec.group_tec_milestone')
    ...
    def _compute_task_count(self):
        task_read_group = self.env['tec.task']._read_group([('milestone_id', 'in', self.ids), ('allow_milestones', '=', True)], ['milestone_id'], ['milestone_id'])
    ...
    def _compute_can_be_marked_as_done(self):
        ...
        if unreached_milestones:
            task_read_group = self.env['tec.task']._read_group(
    ...
    def action_view_tasks(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('tec.action_view_task_from_milestone')
        action['context'] = {'default_tec_id': self.tec_id.id, 'default_milestone_id': self.id}
    ...
    def copy(self, default=None):
        if default is None:
            default = {}
        milestone_copy = super(TecMilestone, self).copy(default)
        if self.tec_id.allow_milestones:
