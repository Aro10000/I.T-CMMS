...
class TecTaskRecurrence(models.Model):
    _name = 'tec.task.recurrence'
    _description = 'Task Recurrence'

    task_ids = fields.One2many('tec.task', 'recurrence_id', copy=False)
    ...
    def _check_recurrence_days(self):
        for tec in self.filtered(lambda p: p.repeat_unit == 'week'):
            if not any([tec.mon, tec.tue, tec.wed, tec.thu, tec.fri, tec.sat, tec.sun]):
    ...
    def _get_recurring_fields(self):
        return ['message_partner_ids', 'company_id', 'description', 'displayed_image_id', 'email_cc',
                ...
                'tec_id', 'display_tec_id', 'tec_privacy_visibility', 'sequence', 'tag_ids', 'recurrence_id',
    ...
    def _new_task_values(self, task):
        ...
        create_values['stage_id'] = task.tec_id.type_ids[0].id if task.tec_id.type_ids else task.stage_id.id
    ...
    def _create_subtasks(self, task, new_task, depth=3):
        ...
        for child in task.child_ids:
            ...
            if child.child_ids and depth > 1:
                ...
                # "grandchildren" tasks
                new_child = self.env['tec.task'].sudo().create(child_values)
                self._create_subtasks(child, new_child, depth=depth - 1)
            else:
                children.append(child_values)
        self.env['tec.task'].sudo().create(children)
    ...
    def _create_next_task(self):
        for recurrence in self:
            ...
            new_task = self.env['tec.task'].sudo().create(create_values)
    ...
    def _cron_create_recurring_tasks(self):
        if not self.env.user.has_group('tec.group_tec_recurring_tasks'):
            return
        today = fields.Date.today()
    ...
    def write(self, vals):
        ...
        res = super(TecTaskRecurrence, self).write(vals)
