...
class ReportTecTaskUser(models.Model):
    _name = "report.tec.task.user"
    _description = "Tasks Analysis"
    _order = 'name desc, tec_id'
    _auto = False
    ...
    tec_id = fields.Many2one('tec.tec', string='Tec', readonly=True)
    ...
    rating_last_value = fields.Float('Rating Value (/5)', group_operator="avg", readonly=True, groups="tec.group_tec_rating")
    rating_avg = fields.Float('Average Rating', readonly=True, group_operator='avg', groups="tec.group_tec_rating")
    ...
    stage_id = fields.Many2one('tec.task.type', string='Stage', readonly=True)
    is_closed = fields.Boolean("Closing Stage", readonly=True, help="Folded in Kanban stages are closing stages.")
    task_id = fields.Many2one('tec.task', string='Tasks', readonly=True)
    active = fields.Boolean(readonly=True)
    tag_ids = fields.Many2many('tec.tags', relation='tec_tags_tec_task_rel',
        column1='tec_task_id', column2='tec_tags_id',
        string='Tags', readonly=True)
    parent_id = fields.Many2one('tec.task', string='Parent Task', readonly=True)
    ancestor_id = fields.Many2one('tec.task', string="Ancestor Task", readonly=True)
    ...
    personal_stage_type_ids = fields.Many2many('tec.task.type', relation='tec_task_user_rel',
        ...
        return f"""
                tec_task t
                    LEFT JOIN rating_rating rt ON rt.res_id = t.id
                        AND rt.res_model = 'tec.task'
                        AND rt.consumed = True
                        AND rt.rating >= {RATING_LIMIT_MIN}
                    LEFT JOIN tec_milestone pm ON pm.id = t.milestone_id
        """

    def _where(self):
        return """
                t.tec_id IS NOT NULL
        """
