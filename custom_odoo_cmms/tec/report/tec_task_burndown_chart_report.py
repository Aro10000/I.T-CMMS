...
class ReportTecTaskBurndownChart(models.AbstractModel):
    _name = 'tec.task.burndown.chart.report'
    ...
    display_tec_id = fields.Many2one('tec.tec', readonly=True)
    is_closed = fields.Boolean("Closing Stage", readonly=True)
    milestone_id = fields.Many2one('tec.milestone', readonly=True)
    partner_id = fields.Many2one('res.partner', string='Customer', readonly=True)
    tec_id = fields.Many2one('tec.tec', readonly=True)
    stage_id = fields.Many2one('tec.task.type', readonly=True)
    user_ids = fields.Many2many('res.users', relation='tec_task_user_rel', column1='task_id', column2='user_id',
                                string='Assignees', readonly=True)

    # Fake field required as used in the filters. It will however be managed through the `tec.task` model.
    has_late_and_unreached_milestone = fields.Boolean(readonly=True)

    # This variable is used in order to distinguish conditions that can be set on `tec.task` and thus being used
    ...
    def _get_group_by_SQL(self, task_specific_domain, count_field, select_terms, from_clause, where_clause,
        ...
        # Build the query on `tec.task` with the domain fields that are linked to that model. This is done in order
        ...
        tec_task_query = self.env['tec.task']._where_calc(task_specific_domain)
        tec_task_from_clause, tec_task_where_clause, tec_task_where_clause_params = tec_task_query.get_sql()
        ...
        field_id = IrModelFieldsSudo.search([('name', '=', 'stage_id'), ('model', '=', 'tec.task')]).id
        ...
        # Insert `WHERE` clause parameter that apply on `tec_task` prior to the one that apply on
        # `tec_task_burndown_chart_report` as the `tec_task` CTE is placed at the beginning of the `SQL`.
        for param in reversed(tec_task_where_clause_params):
        ...
        burndown_chart_query = """
              ...
              all_stage_task_moves AS (
                 SELECT count(*) as %(count_field)s,
                   ...
                   FROM (
                            ...
                            --   message and a mail tracking value on tec.task stage_id.
                            -- * The stage at creation for those for which we do not have any mail message and a
                            --   mail tracking value on tec.task stage_id.
                                                ...
                                                LEFT JOIN (
                                                    mail_message mm
                                                        JOIN mail_tracking_value mtv ON mm.id = mtv.mail_message_id
                                                                                     ...
                                                                                     AND mm.model='tec.task'
                                                                                     AND mm.message_type = 'notification'
                                                        JOIN tec_task_type ptt ON ptt.id = mtv.old_value_integer
                                                ) ON mm.res_id = pt.id
                          ...
                          UNION ALL
                            ...
                            SELECT pt.id as task_id,
                              ...
                              FROM tec_task pt
                                   JOIN tec_task_type ptt ON ptt.id = pt.stage_id
                                   JOIN LATERAL (
                                       SELECT mm.date
              ...
              SELECT (tec_id*10^13 + stage_id*10^7 + to_char(date, 'YYMMDD')::integer)::bigint as id,
                     planned_hours,
                     tec_id,
                     display_tec_id,
                     stage_id,
                ...
                FROM all_stage_task_moves t
                         JOIN LATERAL generate_series(t.date_begin, t.date_end-INTERVAL '1 day', '%(interval)s')
                            AS date ON TRUE
        """ % {
            'task_query_from': tec_task_from_clause,
            'task_query_where': prefix_term('WHERE', tec_task_where_clause),
        ...
        # Replace, in the `FROM` clause generated on `tec_task_burndown_chart_report`, the
        # `tec_task_burndown_chart_report` table name by the burndown_chart_query `SQL` aliased as
        # `tec_task_burndown_chart_report`.
        from_clause = from_clause.replace('"tec_task_burndown_chart_report"', '(%s) AS "tec_task_burndown_chart_report"' % burndown_chart_query, 1)
    ...
    def _determine_domains(self, domain):
        ...
        * A domain that only contains fields that are specific to `tec.task.burndown.chart.report`
        * A domain that only contains fields that are specific to `tec.task`
        ...
        :return: A tuple containing the non `tec.task` specific domain and the `tec.task` specific domain.
        """
    ...
    def _read_group_raw(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        # --- Below code is custom
        # --- As the report is base on `tec.task` we flush that specific model

        # self._flush_search(domain, fields=fnames + groupby_fields)
        self.env['tec.task']._flush_search(task_specific_domain, fields=self.task_specific_fields)
