...
export class TecTaskKanbanRenderer extends KanbanRenderer {
    setup() {
        super.setup();
        this.userService = useService('user');
        this.action = useService('action');

        this.isTecManager = false;
    ...
    async onWillStart() {
        if (!this.props.list.isGroupedByPersonalStages) { // no need to check it if the group by is personal stages
            this.isTecManager = await this.userService.hasGroup('tec.group_tec_manager');
        }
    }

    canCreateGroup() {
        return super.canCreateGroup() && (!this.props.list.isGroupedByStage || this.isTecManager) || this.props.list.isGroupedByPersonalStages;
    }

    canDeleteGroup(group) {
        return super.canDeleteGroup(group) && (!this.props.list.isGroupedByStage || this.isTecManager) || this.props.list.isGroupedByPersonalStages;
    }

    canEditGroup(group) {
        return super.canEditGroup(group) && (!this.props.list.isGroupedByStage || this.isTecManager) || this.props.list.isGroupedByPersonalStages;
    }
    ...
    editGroup(group) {
        ...
        const context = Object.assign({}, group.context, {
            form_view_ref: 'tec.personal_task_type_edit',
        });
