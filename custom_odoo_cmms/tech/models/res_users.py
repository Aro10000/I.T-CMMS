...
class Users(models.Model):
    ...
    def _systray_get_tech_event_domain(self):
        ...
        return ['&', '|',
                ...
                ('attendez_ids.partner_id', '=', self.env.user.partner_id.id)]

    @api.model
    def systray_get_activities(self):
        res = super(Users, self).systray_get_activities()

        meetings_lines = self.env['tech.event'].search_read(
            self._systray_get_tech_event_domain(),
            ['id', 'start', 'name', 'allday', 'attendez_status'],
            order='start')
        meetings_lines = [line for line in meetings_lines if line['attendez_status'] != 'declined']
        if meetings_lines:
            meeting_label = _("Today's Meetings")
            meetings_systray = {
                'id': self.env['ir.model']._get('tech.event').id,
                'type': 'meeting',
                'name': meeting_label,
                'model': 'tech.event',
                'icon': modules.module.get_module_icon(self.env['tech.event']._original_module),
