...
class AlarmManager(models.AbstractModel):
    ...
    def _get_next_potential_limit_alarm(self, alarm_type, seconds=None, partners=None):
        ...
        for model_name in ('tech.alarm', 'tech.event', 'tech.recurrence'):
            self.env[model_name].flush_model()

        result = {}
        delta_request = """
            SELECT
                rel.tech_event_id, max(alarm.duration_minutes) AS max_delta,min(alarm.duration_minutes) AS min_delta
            FROM
                tech_alarm_tech_event_rel AS rel
            LEFT JOIN tech_alarm AS alarm ON alarm.id = rel.tech_alarm_id
            WHERE alarm.alarm_type = %s
            GROUP BY rel.tech_event_id
        """
        base_request = """
                    SELECT
                        cal.id,
                    ...
                    FROM
                        tech_event AS cal
                    RIGHT JOIN calcul_delta ON calcul_delta.tech_event_id = cal.id
                    LEFT JOIN tech_recurrence as rrule ON rrule.id = cal.recurrence_id
             """

        filter_user = """
                RIGHT JOIN tech_event_res_partner_rel AS part_rel ON part_rel.tech_event_id = cal.id
        ...
        events = self.env['tech.event'].browse(result)
        result = {
            key: result[key]
    ...
    def _get_events_by_alarm_to_notify(self, alarm_type):
        ...
        design. The attendezs receive an invitation for any new event
        already.
        """
        lastcall = self.env.context.get('lastcall', False) or fields.date.today() - relativedelta(weeks=1)
        self.env.cr.execute('''
            SELECT "alarm"."id", "event"."id"
              FROM "tech_event" AS "event"
              JOIN "tech_alarm_tech_event_rel" AS "event_alarm_rel"
                ON "event"."id" = "event_alarm_rel"."tech_event_id"
              JOIN "tech_alarm" AS "alarm"
                ON "event_alarm_rel"."tech_alarm_id" = "alarm"."id"
             WHERE (
                   "alarm"."alarm_type" = %s
               AND "event"."active"
    ...
    def _send_reminder(self):
        ...
        events = self.env['tech.event'].browse(event_ids)
        attendezs = events.attendez_ids.filtered(lambda a: a.state != 'declined')
        alarms = self.env['tech.alarm'].browse(events_by_alarm.keys())
        for alarm in alarms:
            alarm_attendezs = attendezs.filtered(lambda attendez: attendez.event_id.id in events_by_alarm[alarm.id])
            alarm_attendezs.with_context(
                mail_notify_force_send=True,
                tech_template_ignore_recurrence=True
    ...
    def get_next_notif(self):
        ...
        for event_id in all_meetings:
            ...
            meeting = self.env['tech.event'].browse(event_id)
            in_date_format = fields.Datetime.from_string(meeting.start)
            last_found = self.do_check_alarm_for_one_date(in_date_format, meeting, max_delta, time_limit, 'notification', after=partner.tech_last_notif_ack)
            if last_found:
    ...
    def do_notif_reminder(self, alert):
        alarm = self.env['tech.alarm'].browse(alert['alarm_id'])
        meeting = self.env['tech.event'].browse(alert['event_id'])
    ...
    def _notify_next_alarm(self, partner_ids):
        ...
        for user in users:
            ...
            notifications.append([user.partner_id, 'tech.alarm', notif])
