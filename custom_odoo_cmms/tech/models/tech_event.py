...
from odoo.addons.tech.models.tech_attendez import Attendez
from odoo.addons.tech.models.tech_recurrence import (
    weekday_to_field,
...
class TechMeeting(models.Model):
    _name = 'tech.event'
    _description = "Tech Event"
    _order = "start desc"
    _inherit = ["mail.thread"]

    DISCUSS_ROUTE = 'tech/join_videocall'
    ...
    def default_get(self, fields):
        ...
        defaults = super(TechMeeting, self).default_get(fields)
        ...
        if 'res_model_id' not in defaults and 'res_model_id' in fields and \
                self.env.context.get('active_model') and self.env.context['active_model'] != 'tech.event':
    ...
    def _default_partners(self):
        """ When active_model is res.partner, the current partners should be attendezs """
    ...
    # description
    name = fields.Char('TechMeeting Subject', required=True)
    ...
    videocall_location = fields.Char('TechMeeting URL', compute='_compute_videocall_location', store=True, copy=True)
    ...
    is_organizer_alone = fields.Boolean(compute='_compute_is_organizer_alone', string="Is the Organizer Alone",
        ...
        the event (only if the organizer is not the only attendez)""")
    # filtering
    ...
    cat_ids = fields.Many2many(
        'tech.event.type', 'meeting_category_rel', 'event_id', 'type_id', 'Tags')
    # timing
    ...
    # messaging
    activity_ids = fields.One2many('mail.activity', 'tech_event_id', string='Activities')
    # attendezs
    attendez_ids = fields.One2many(
        'tech.attendez', 'event_id', 'Participant')
    attendez_status = fields.Selection(
        Attendez.STATE_SELECTION, string='Attendez Status', compute='_compute_attendez')
    partner_ids = fields.Many2many(
        'res.partner', 'tech_event_res_partner_rel',
        string='Attendezs', default=_default_partners)
    invalid_email_partner_ids = fields.Many2many('res.partner', compute='_compute_invalid_email_partner_ids')
    # alarms
    alarm_ids = fields.Many2many(
        'tech.alarm', 'tech_alarm_tech_event_rel',
        string='Reminders', ondelete="restrict",
        help="Notifications sent to all attendezs to remind of the meeting.")
    # RECURRENCE FIELD
    recurrency = fields.Boolean('Recurrent')
    recurrence_id = fields.Many2one(
        'tech.recurrence', string="Recurrence Rule")
    ...
    # a `tech.recurrence.rule` will be dynamically created.
    ...
    @api.depends('partner_id', 'attendez_ids')
    def _compute_is_organizer_alone(self):
        """
            Check if the organizer of the event is the only one who has accepted the event.
            It does not apply if the organizer is the only attendez of the event because it
            would represent a personnal event.
            The goal of this field is to highlight to the user that the others attendezs are
            not available for this event.
        """
        for event in self:
            organizer = event.attendez_ids.filtered(lambda a: a.partner_id == event.partner_id)
            all_declined = not any((event.attendez_ids - organizer).filtered(lambda a: a.state != 'declined'))
            event.is_organizer_alone = len(event.attendez_ids) > 1 and all_declined
    ...
    def _onchange_date(self):
        ...
        for event in self:
            if event.stop_date and event.start_date:
                event.with_context(is_tech_event_new=True).write({
    ...
    def _inverse_dates(self):
        """ This method is used to set the start and stop values of all day events.
            The tech view needs date_start and date_stop values to display correctly the allday events across
            several days. As the user edit the {start,stop}_date fields when allday is true,
            this inverse method is needed to update the  start/stop value and have a relevant tech view.
        """
        for meeting in self:
            if meeting.allday:
                # Convention break:
                ...
                # because fulltech just drops times for full day events.
    ...
    def _compute_attendez(self):
        mapped_attendezs = self._find_attendez_batch()
        for meeting in self:
            attendez = mapped_attendezs[meeting.id]
            meeting.attendez_status = attendez.state if attendez else 'needsAction'
        ...
        for meeting in self:
            if not meeting.allday and meeting.start and meeting.stop and meeting.stop < meeting.start:
                raise ValidationError(
                    ...
                    _("TechMeeting '%(name)s' starts '%(start_datetime)s' and ends '%(end_datetime)s'",
                      name=meeting.name,
    ...
    def _compute_recurrence(self):
        ...
        defaults = self.env['tech.recurrence'].default_get(recurrence_fields)
    ...
    def create(self, vals_list):
        ...
        self = self.with_context(is_tech_event_new=True)
        ...
        if meeting_activity_type and not defaults.get('activity_ids'):
            for values in vals_list:
                # created from tech: try to create an activity on the related record
        ...
        # Add commands to create attendezs from partners (if present) if no attendez command
        ...
        vals_list = [
            dict(vals, attendez_ids=self._attendezs_values(vals.get('partner_ids', default_partners_ids)))
            if not vals.get('attendez_ids')
            else vals
            for vals in vals_list
        ]
        recurrence_fields = self._get_recurrent_fields()
        ...
        events.filtered(lambda event: event.start > fields.Datetime.now()).attendez_ids._send_mail_to_attendezs(
            self.env.ref('tech.tech_template_meeting_invitation', raise_if_not_found=False)
        )
        ...
        return events.with_context(is_tech_event_new=False)

    def _compute_field_value(self, field):
        if field.compute_sudo:
            return super(TechMeeting, self.with_context(prefetch_fields=False))._compute_field_value(field)
    ...
    def write(self, values):
        detached_events = self.env['tech.event']
        ...
        if 'partner_ids' in values:
            values['attendez_ids'] = self._attendezs_values(values['partner_ids'])
            update_alarms = True
        ...
        time_fields = self.env['tech.event']._get_time_fields()
        ...
        previous_attendezs = self.attendez_ids
        ...
        # Notify attendezs if there is an alarm on the modified event, or if there was an alarm
        ...
        if not self.env.context.get('dont_notify') and update_alarms:
            self._setup_alarms()
        attendez_update_events = self.filtered(lambda ev: ev.user_id and ev.user_id != self.env.user)
        if update_time and attendez_update_events:
            ...
            attendez_update_events.attendez_ids.filtered(lambda att: self.user_id.partner_id == att.partner_id).write(
                {'state': 'needsAction'})

        current_attendezs = self.filtered('active').attendez_ids
        if 'partner_ids' in values:
            # we send to all partners and not only the new ones
            (current_attendezs - previous_attendezs)._send_mail_to_attendezs(
                self.env.ref('tech.tech_template_meeting_invitation', raise_if_not_found=False)
            )
        if not self.env.context.get('is_tech_event_new') and 'start' in values:
            start_date = fields.Datetime.to_datetime(values.get('start'))
            # Only notify on future events
            if start_date and start_date >= fields.Datetime.now():
                (current_attendezs & previous_attendezs).with_context(
                    tech_template_ignore_recurrence=not update_recurrence
                )._send_mail_to_attendezs(
                    self.env.ref('tech.tech_template_meeting_changedate', raise_if_not_found=False)
                )
    ...
    def name_get(self):
        ...
        shown = self - hidden
        shown_names = super(TechMeeting, shown).name_get()
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        if not self.env.su and private_fields:
            ...
            return super(TechMeeting, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby,
                                                   lazy=lazy)
        return super(TechMeeting, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby,
                                               lazy=lazy)
    ...
    def unlink(self):
        ...
        # Get concerned attendezs to notify them if there is an alarm on the unlinked events,
        ...
        recurrences = self.env["tech.recurrence"].search([
        ...
        # Notify the concerned attendezs (must be done after removing the events)
        self.env['tech.alarm_manager']._notify_next_alarm(partner_ids)
        return result

    def copy(self, default=None):
        """When an event is copied, the attendezs should be recreated to avoid sharing the same attendez records
         between copies
         """
        self.ensure_one()
        if not default:
            default = {}
        # We need to make sure that the attendez_ids are recreated with new ids to avoid sharing attendezs between events
        # The copy should not have the same attendez status than the original event
        default.update(partner_ids=[Command.set([])], attendez_ids=[Command.set([])])
        copied_event = super().copy(default)
        copied_event.write({'partner_ids': [(Command.set(self.partner_ids.ids))]})
        return copied_event

    def _attendezs_values(self, partner_commands):
        """
        :param partner_commands: ORM commands for partner_id field (0 and 1 commands not supported)
        :return: associated attendez_ids ORM commands
        """
        attendez_commands = []
        ...
        for command in partner_commands:
            ...
            if op in (2, 3, Command.delete, Command.unlink):  # Remove partner
                ...
                removed_partner_ids += set(self.partner_ids.ids) - set(
                    command[2])  # Don't recreate attendez if partner already attend the event
        ...
        if not self:
            attendezs_to_unlink = self.env['tech.attendez']
        else:
            attendezs_to_unlink = self.env['tech.attendez'].search([
                ('event_id', 'in', self.ids),
                ('partner_id', 'in', removed_partner_ids),
            ])
        attendez_commands += [[2, attendez.id] for attendez in attendezs_to_unlink]  # Removes and delete

        attendez_commands += [
            [0, 0, dict(partner_id=partner_id)]
            for partner_id in added_partner_ids
        ]
        return attendez_commands

    def _create_videocall_channel(self):
        if self.recurrency:
            # check if any of the events have videocall_channel_id, if not create one
            event_with_channel = self.env['tech.event'].search([
    ...
    def _create_videocall_channel_id(self, name, partner_ids):
        videocall_channel_id = self.env['mail.channel'].create_group(partner_ids,
                                                                     default_display_mode='video_full_screen',
                                                                     name=name)
        # if recurrent event, set channel to all other records of the same recurrency
        if self.recurrency:
            recurrent_events_without_channel = self.env['tech.event'].search([
    ...
    def action_open_tech_event(self):
    ...
    def action_sendmail(self):
        ...
        if email:
            for meeting in self:
                meeting.attendez_ids._send_mail_to_attendezs(
                    self.env.ref('tech.tech_template_meeting_invitation', raise_if_not_found=False)
                )
        return True
    ...
    def action_open_composer(self):
        if not self.partner_ids:
            raise UserError(_("There are no attendezs on these events"))
        template_id = self.env['ir.model.data']._xmlid_to_res_id('tech.tech_template_meeting_update',
    ...
    def action_mass_deletion(self, recurrence_update_setting):
        ...
        if recurrence_update_setting == 'all_events':
            events = self.recurrence_id.tech_event_ids
            self.recurrence_id.unlink()
            events.unlink()
        elif recurrence_update_setting == 'future_events':
            future_events = self.recurrence_id.tech_event_ids.filtered(lambda ev: ev.start >= self.start)
            future_events.unlink()

    def action_mass_archive(self, recurrence_update_setting):
        """
        The aim of this action purpose is to be called from sync tech module when mass deletion is not possible.
        """
        self.ensure_one()
        if recurrence_update_setting == 'all_events':
            self.recurrence_id.tech_event_ids.write(self._get_archive_values())
        elif recurrence_update_setting == 'future_events':
            ...
            if len(self.recurrence_id.tech_event_ids) == 0:
                self.recurrence_id.unlink()
    ...
    def _get_attendez_emails(self):
        """ Get comma-separated attendez email addresses. """
        self.ensure_one()
        return ",".join([e for e in self.attendez_ids.mapped("email") if e])
    ...
    def _setup_alarms(self):
        ...
        cron = self.env.ref('tech.ir_cron_scheduler_alarm').sudo()
        alarm_types = self._get_trigger_alarm_types()
        events_to_notify = self.env['tech.event']
        ...
        for event in self:
            ...
            if any(alarm.alarm_type == 'notification' for alarm in event.alarm_ids):
                # filter events before notifying attendezs through tech_alarm_manager
                events_to_notify |= event.filtered(lambda ev: ev.alarm_ids and ev.stop >= fields.Datetime.now())
        if events_to_notify:
            self.env['tech.alarm_manager']._notify_next_alarm(events_to_notify.partner_ids.ids)
    ...
    def _apply_recurrence_values(self, values, future=True):
        ...
        if not values:
            return self.browse()
        recurrence_vals = []
        to_update = self.env['tech.recurrence']
        for event in self:
            if not event.recurrence_id:
                recurrence_vals += [dict(values, base_event_id=event.id, tech_event_ids=[(4, event.id)])]
            elif future:
                to_update |= event.recurrence_id._split_from(event, values)
        self.write({'recurrency': True, 'follow_recurrence': True})
        to_update |= self.env['tech.recurrence'].create(recurrence_vals)
    ...
    def _break_recurrence(self, future=True):
        ...
        """
        recurrences_to_unlink = self.env['tech.recurrence']
        detached_events = self.env['tech.event']
        for event in self:
            ...
            if future:
                ...
                detached_events |= recurrence.tech_event_ids
                recurrence.tech_event_ids.recurrence_id = False
    ...
    def _get_archive_values(self):
        """ Return parameters for archiving events in tech module. """
        return {'active': False}
    ...
    def change_attendez_status(self, status, recurrence_update_setting):
        self.ensure_one()
        if recurrence_update_setting == 'all_events':
            events = self.recurrence_id.tech_event_ids
        elif recurrence_update_setting == 'future_events':
            events = self.recurrence_id.tech_event_ids.filtered(lambda ev: ev.start >= self.start)
        else:
            events = self
        attendez = events.attendez_ids.filtered(lambda x: x.partner_id == self.env.user.partner_id)
        if status == 'accepted':
            all_events = recurrence_update_setting == 'all_events'
            return attendez.with_context(all_events=all_events).do_accept()
        if status == 'declined':
            return attendez.do_decline()
        return attendez.do_tentative()

    def find_partner_customer(self):
        self.ensure_one()
        return next(
            (attendez.partner_id for attendez in self.attendez_ids.sorted('create_date')
             if attendez.partner_id != self.user_id.partner_id),
            self.env['tech.attendez']
        )
    ...
    def _find_attendez_batch(self):
        """ Return the first attendez where the user connected has been invited
            or the attendez selected in the filter that is the owner
            from all the meeting_ids in parameters.
        """
        result = defaultdict(lambda: self.env['tech.attendez'])
        self_attendezs = self.attendez_ids.filtered(lambda a: a.partner_id == self.env.user.partner_id)
        for attendez in self_attendezs:
            result[attendez.event_id.id] = attendez
        remaining_events = self - self_attendezs.event_id

        events_checked_partners = self.env['tech.filters'].search([
            ('user_id', '=', self.env.user.id),
            ('partner_id', 'in', remaining_events.attendez_ids.partner_id.ids),
            ('partner_checked', '=', True)
        ]).partner_id
        filter_events = self.env['tech.event']
        for event in remaining_events:
            event_partners = event.attendez_ids.partner_id
            ...
            if event.partner_id in event_checked_partners and event.partner_id in event_partners:
                filter_events |= event
                result[event.id] = event.attendez_ids.filtered(
                    lambda attendez: attendez.partner_id == event.partner_id)[:1]
        remaining_events -= filter_events

        for event in remaining_events:
            event_checked_partners = events_checked_partners & event_partners
            attendez = event.attendez_ids.filtered(
                lambda a: a.partner_id in event_checked_partners and a.state != "needsAction")
            result[event.id] = attendez[:1]
        return result

    # YTI TODO MASTER: Remove deprecated method
    def _find_attendez(self):
        """ Return the first attendez where the user connected has been invited
            or the attendez selected in the filter that is the owner
            from all the meeting_ids in parameters.
        """
        self.ensure_one()
        return self._find_attendez_batch()[self.id]
    ...
    def _get_ics_file(self):
        """ Returns iTech file for the event invitation.
        ...
        for meeting in self:
            ...
            if not is_html_empty(meeting.description):
                if 'appointment_type_id' in meeting._fields and self.appointment_type_id:
                    # convert_online_event_desc_to_text method for correct data formatting in external techs
            ...
            for attendez in meeting.attendez_ids:
                attendez_add = event.add('attendez')
                attendez_add.value = u'MAILTO:' + (attendez.email or u'')
    ...
    def convert_online_event_desc_to_text(self, description):
        """
        We can sync the tech events with google tech, iCal and Outlook, and we
        ...
        techs). Note that for regular (offline) events, we simply use the standard
