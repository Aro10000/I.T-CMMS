...
class RecurrenceRule(models.Model):
    _name = 'tech.recurrence'
    _description = 'Event Recurrence Rule'

    name = fields.Char(compute='_compute_name', store=True)
    base_event_id = fields.Many2one(
        'tech.event', ondelete='set null', copy=False)  # store=False ?
    tech_event_ids = fields.One2many('tech.event', 'recurrence_id')
    ...
    @api.depends('tech_event_ids.start')
    def _compute_dtstart(self):
        groups = self.env['tech.event'].read_group([('recurrence_id', 'in', self.ids)], ['start:min'], ['recurrence_id'])
        start_mapping = {
    ...
    def _reconcile_events(self, ranges):
        ...
        synced_events = self.tech_event_ids.filtered(lambda e: e._range() in ranges)
    ...
    def _apply_recurrence(self, specific_values_creation=None, no_send_edit=False, generic_values_creation=None):
        ...
        events = self.tech_event_ids - keep
        detached_events = self._detach_events(events)
        self.env['tech.event'].with_context(no_mail_to_attendezs=True, mail_create_nolog=True).create(event_vals)
    ...
    def _split_from(self, event, recurrence_values=None):
        ...
        return self.create({
            ...
            **recurrence_values,
            ...
            'tech_event_ids': [(6, 0, detached_events.ids)],
            'count': max(count, 1),
        })
    ...
    def _write_events(self, values, dtstart=None):
        ...
        """
        events = self._get_events_from(dtstart) if dtstart else self.tech_event_ids
        return events.with_context(no_mail_to_attendezs=True, dont_notify=True).write(dict(values, recurrence_update='self_only'))

    def _rrule_serialize(self):
        """
        Compute rule string according to value type RECUR of iTech
    ...
    def _get_start_of_period(self, dt):
        ...
        # This is a hack to avoid duplication of events (for example on google tech).
    ...
    def _get_first_event(self, include_outliers=False):
        if not self.tech_event_ids:
            return self.env['tech.event']
        events = self.tech_event_ids.sorted('start')
        if not include_outliers:
            events -= self._get_outliers()
        return events[:1]

    def _get_outliers(self):
        synced_events = self.env['tech.event']
        for recurrence in self:
            if recurrence.tech_event_ids:
                start = min(recurrence.tech_event_ids.mapped('start'))
                starts = set(recurrence._get_occurrences(start))
                synced_events |= recurrence.tech_event_ids.filtered(lambda e: e.start in starts)
        return self.tech_event_ids - synced_events
    ...
    def _get_events_from(self, dtstart):
        return self.env['tech.event'].search([
            ('id', 'in', self.tech_event_ids.ids),
            ('start', '>=', dtstart)
        ])
    ...
    def _is_allday(self):
        ...
        """
        score = sum(1 if e.allday else -1 for e in self.tech_event_ids)
        return score >= 0
