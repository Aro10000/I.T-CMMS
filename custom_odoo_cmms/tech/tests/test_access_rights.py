...
class TestAccessRights(TransactionCase):
    ...
    def create_event(self, user, **values):
        return self.env['tech.event'].with_user(user).create({
            'name': 'Event',
    ...
    def test_read_group_public(self):
        ...
        data = self.env['tech.event'].with_user(self.raoul).read_group([('id', '=', event.id)], fields=['start'], groupby='start')
        self.assertTrue(data, "It should be able to read group")
        data = self.env['tech.event'].with_user(self.raoul).read_group([('id', '=', event.id)], fields=['name'],
                                                                           groupby='name')
        self.assertTrue(data, "It should be able to read group")

    def test_read_group_private(self):
        event = self.create_event(self.john, privacy='private')
        result = self.env['tech.event'].with_user(self.raoul).read_group([('id', '=', event.id)], fields=['name'], groupby='name')
        self.assertFalse(result, "Private events should not be fetched")


    def test_read_group_agg(self):
        event = self.create_event(self.john)
        data = self.env['tech.event'].with_user(self.raoul).read_group([('id', '=', event.id)], fields=['start'], groupby='start:week')
        self.assertTrue(data, "It should be able to read group")
    ...
    def test_private_attendez(self):
        event = self.create_event(
            self.john,
            privacy='private',
            location='in the Sky',
    ...
    def test_meeting_edit_access_notification_handle_in_odoo(self):
        ...
        meeting = self.env['tech.event'].with_user(self.raoul).create({
            'name': 'Test Meeting',
            'start': datetime.now(),
        ...
        # it indicates that there might be an issue with access rights for meeting attendezs.
