...
class TestCreateRecurrentEvents(TestRecurrentEvents):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.event = cls.env['tech.event'].create({
    ...
    def test_weekly_count(self):
        ...
        recurrence = self.env['tech.recurrence'].search([('base_event_id', '=', self.event.id)])
        events = recurrence.tech_event_ids
    ...
    def test_yearly_count(self):
        ...
        events = self.event.recurrence_id.tech_event_ids
        self.assertEqual(len(events), 2, "It should have 3 events in the recurrence")
        self.assertEventDates(events, [
    ...
    def test_dst_timezone(self):
        ...
        recurrence = self.env['tech.recurrence'].search([('base_event_id', '=', self.event.id)])
        self.assertEventDates(recurrence.tech_event_ids, [
    ...
    def test_videocall_recurrency(self):
        ...
        recurrent_events = self.event.recurrence_id.tech_event_ids
        detached_events = self.event.recurrence_id.tech_event_ids - self.event
...
class TestUpdateRecurrentEvents(TestRecurrentEvents):
    ...
    def setUpClass(cls):
        super().setUpClass()
        event = cls.env['tech.event'].create({
            'name': 'Recurrent Event',
        ...
        cls.events = event.recurrence_id.tech_event_ids.sorted('start')
    ...
    def test_shift_future(self):
        ...
        self.assertEventDates(self.recurrence.tech_event_ids, [
        ...
        self.assertEventDates(new_recurrence.tech_event_ids, [
    ...
    def test_shift_all(self):
        event = self.events[1]
        self.assertEventDates(event.recurrence_id.tech_event_ids, [
            (datetime(2019, 10, 22, 1, 0), datetime(2019, 10, 24, 18, 0)),
            (datetime(2019, 10, 29, 1, 0), datetime(2019, 10, 31, 18, 0)),
        ...
        recurrence = self.env['tech.recurrence'].search([])
        self.assertEventDates(recurrence.tech_event_ids, [
            (datetime(2019, 10, 26, 1, 0), datetime(2019, 10, 29, 18, 0)),
            (datetime(2019, 11, 2, 1, 0), datetime(2019, 11, 5, 18, 0)),
            (datetime(2019, 11, 9, 1, 0), datetime(2019, 11, 12, 18, 0)),
    ...
    def test_shift_all_base_inactive(self):
        ...
        self.assertFalse(self.recurrence.tech_event_ids, "Inactive event should not create recurrent events")
    ...
    def test_update_recurrence_future(self):
        ...
        events = event.recurrence_id.tech_event_ids.sorted('start')
    ...
    def test_update_name_future(self):
        ...
        new_recurrence = self.env['tech.recurrence'].search([('id', '>', self.events[0].recurrence_id.id)])
        ...
        for event in new_recurrence.tech_event_ids:
            self.assertEqual(event.name, 'New name')

        # update the base event
        new_events = new_recurrence.tech_event_ids.sorted('start')
        new_events[0].write({
            'name': 'Old name',
            'recurrence_update': 'future_events'
        })
    ...
    def test_all_day_shift(self):
        recurrence = self.env['tech.event'].create({
            'name': 'Recurrent Event',
        ...
        events = recurrence.tech_event_ids.sorted('start')
        event = events[1]
        event.write({
            'recurrence_update': 'future_events',
            'start': event.start + relativedelta(days=4),
    ...
    def test_update_name_all(self):
        ...
        old_events = old_recurrence.tech_event_ids - self.events[0]
        self.events[0].write({
            'name': 'New name',
            'recurrence_update': 'all_events',
            'count': '5'
        })
        new_recurrence = self.env['tech.recurrence'].search([('id', '>', old_recurrence.id)])
...
class TestUpdateMonthlyByDate(TestRecurrentEvents):
    ...
    def test_update_all(self):
        ...
        recurrence = self.env['tech.recurrence'].search([('day', '=', 25)])
        self.assertEventDates(recurrence.tech_event_ids, [
