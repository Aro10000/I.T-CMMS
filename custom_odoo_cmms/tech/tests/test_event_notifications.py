...
class TestEventNotifications(TransactionCase, MailCase, CronMixinCase):
    ...
    def test_message_invite_allday(self):
        with self.assertSinglePostNotifications([{'partner': self.partner, 'type': 'inbox'}], {
            ...
            self.env['tech.event'].with_context(mail_create_nolog=True).create([{
                'name': 'Meeting',
                'allday': True,
    ...
    def test_message_datetime_changed(self):
        self.event.partner_ids = self.partner
        "Invitation to Presentation of the new Tech"
        with self.assertSinglePostNotifications([{'partner': self.partner, 'type': 'inbox'}], {
            'message_type': 'auto_comment',
            'subtype': 'mail.mt_note',
        }):
    ...
    def test_bus_notif(self):
        alarm = self.env['tech.alarm'].create({
            'name': 'Alarm',
        ...
        with patch.object(fields.Datetime, 'now', lambda: now):
            with self.assertBus([(self.env.cr.dbname, 'res.partner', self.partner.id)], [
                ...
                self.event.with_context(no_mail_to_attendezs=True).write({
    ...
    def test_email_alarm(self):
        now = fields.Datetime.now()
        with self.capture_triggers('tech.ir_cron_scheduler_alarm') as capt:
            alarm = self.env['tech.alarm'].create({
                'name': 'Alarm',
                'alarm_type': 'email',
                'interval': 'minutes',
        ...
        with patch.object(fields.Datetime, 'now', lambda: now):
            with self.assertSinglePostNotifications([{'partner': self.partner, 'type': 'inbox'}], {
                ...
                self.env['tech.alarm_manager'].with_context(lastcall=now - relativedelta(minutes=15))._send_reminder()

    def test_notification_event_timezone(self):
        """
            Check the domain that decides when tech events should be notified to the user.
        """
        def search_event():
            return self.env['tech.event'].search(self.env['res.users']._systray_get_tech_event_domain())
