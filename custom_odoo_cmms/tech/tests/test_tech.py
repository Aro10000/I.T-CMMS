...
class TestTech(SavepointCaseWithUserDemo):

    def setUp(self):
        super(TestTech, self).setUp()

        self.TechEvent = self.env['tech.event']
        # In Order to test tech, I will first create One Simple Event with real data
        self.event_tech_presentation = self.TechEvent.create({
            'privacy': 'private',
    ...
    def test_event_order(self):
        ...
        events = self.TechEvent.search(domain, order='name')
        self.assertEqual(events.mapped('name'), ['bar', 'bar', 'foo', 'foo'])
        events = self.TechEvent.search(domain, order='name desc')
        self.assertEqual(events.mapped('name'), ['foo', 'foo', 'bar', 'bar'])

        # sort them by start date only
        events = self.TechEvent.search(domain, order='start')
        self.assertEqual(events.mapped('start'), (foo1 + bar1 + foo2 + bar2).mapped('start'))
        events = self.TechEvent.search(domain, order='start desc')
        self.assertEqual(events.mapped('start'), (foo2 + bar2 + bar1 + foo1).mapped('start'))

        # sort them by name then start date
        events = self.TechEvent.search(domain, order='name asc, start asc')
        self.assertEqual(list(events), [bar1, bar2, foo1, foo2])
        events = self.TechEvent.search(domain, order='name asc, start desc')
        self.assertEqual(list(events), [bar2, bar1, foo2, foo1])
        events = self.TechEvent.search(domain, order='name desc, start asc')
        self.assertEqual(list(events), [foo1, foo2, bar1, bar2])
        events = self.TechEvent.search(domain, order='name desc, start desc')
        self.assertEqual(list(events), [foo2, foo1, bar2, bar1])

        # sort them by start date then name
        events = self.TechEvent.search(domain, order='start asc, name asc')
        self.assertEqual(list(events), [foo1, bar1, bar2, foo2])
        events = self.TechEvent.search(domain, order='start asc, name desc')
        self.assertEqual(list(events), [foo1, bar1, foo2, bar2])
        events = self.TechEvent.search(domain, order='start desc, name asc')
        self.assertEqual(list(events), [bar2, foo2, bar1, foo1])
        events = self.TechEvent.search(domain, order='start desc, name desc')
    ...
    def test_event_activity(self):
        ...
        test_event = self.env['tech.event'].with_user(test_user).with_context(
        ...
        self.assertEqual(self.env['tech.event'], self.env['tech.event'].search([('name', '=', test_name)]))

        # create using active_model keys
        test_event = self.env['tech.event'].with_user(self.user_demo).with_context(
    ...
    def test_recurring_around_dst(self):
        ...
        start_recurring_dates = m.recurrence_id.tech_event_ids.sorted('start').mapped('start')
    ...
    def test_recurring_ny(self):
        ...
        f = Form(self.TechEvent.with_context(tz='US/Eastern'))
        f.name = 'test'
    ...
    def test_event_activity_timezone(self):
        ...
        tech_event = self.env['tech.event'].create({
        ...
        tech_event.with_context({'tz': 'Australia/Brisbane'}).write({
            'start': '2018-11-12 21:00:00',
        })
    ...
    def test_event_allday_activity_timezone(self):
        ...
        # Also, read the comment in the code at tech.event._inverse_dates
        activty_type = self.env['mail.activity.type'].create({
            'name': 'Meeting',
            'category': 'meeting'
        })
        ...
        # Check output in the user's tz
        # write on the event to trigger sync of activities
        tech_event.with_context({'tz': 'Pacific/Honolulu'}).write({
            'start': '2018-10-16 00:00:00',
            'start_date': '2018-10-16',
        })
    ...
    def test_event_creation_mail(self):
        """
        Check that mail are sent to the attendezs on event creation
        Check that mail are sent to the added attendezs on event edit
        Check that mail are NOT sent to the attendezs when the event date is past
        Check that mail have extra attachement added by the user
        """

        def _test_one_mail_per_attendez(self, partners):
            # check that every attendez receive a (single) mail for the event
        ...
        self.env.ref('tech.tech_template_meeting_invitation').attachment_ids = attachment
        ...
        _test_one_mail_per_attendez(self, partners)
    ...
    def test_event_creation_sudo_other_company(self):
        ...
        self.TechEvent.with_user(web_user).with_company(web_company).sudo().create({
            'name': "Test",
            'allday': False,
            'recurrency': False,
    ...
    def test_meeting_creation_from_partner_form(self):
        """ When going from a partner to the Tech and adding a meeting, both current user and partner
         should be attendezs of the event """
        tech_action = self.partner_demo.schedule_meeting()
        event = self.env['tech.event'].with_context(tech_action['context']).create({
            'name': 'Super Meeting',
            'start': datetime(2020, 12, 13, 17),
            'stop': datetime(2020, 12, 13, 22),
        })
        self.assertEqual(len(event.attendez_ids), 2)
        self.assertTrue(self.partner_demo in event.attendez_ids.mapped('partner_id'))
        self.assertTrue(self.env.user.partner_id in event.attendez_ids.mapped('partner_id'))
...
class TestTechTours(HttpCaseWithUserDemo):
    def test_tech_month_view_start_hour_displayed(self):
        """ Test that the time is displayed in the month view. """
        self.start_tour("/web", 'tech_appointments_hour_tour', login="demo")

    def test_tech_delete_tour(self):
        """
            Check that we can delete events with the "Everybody's techs" filter.
        """
        ...
        event = self.env['tech.event'].with_user(user_admin).create({
            'name': 'Test Event',
        ...
        action_id = self.env.ref('tech.action_tech_event')
        url = "/web#action=" + str(action_id.id) + '&view_type=tech'
        self.start_tour(url, 'test_tech_delete_tour', login='admin')
        event = self.env['tech.event'].search([('name', '=', 'Test Event')])
        self.assertFalse(event) # Check if the event has been correctly deleted

    def test_tech_decline_tour(self):
        """
        ...
        action_id = self.env.ref('tech.action_tech_event')
        url = "/web#action=" + str(action_id.id) + '&view_type=tech'
        self.start_tour(url, 'test_tech_decline_tour', login='demo')
        attendez = self.env['tech.attendez'].search([('event_id', '=', event.id), ('partner_id', '=', user_demo.partner_id.id)])
        self.assertEqual(attendez.state, 'declined') # Check if the event has been correctly declined

    def test_tech_decline_with_everybody_filter_tour(self):
        """
            Check that we can decline events with the "Everybody's techs" filter.
        """
        user_admin = self.env.ref('base.user_admin')
        user_demo = self.user_demo
        start = datetime.combine(date.today(), datetime.min.time()).replace(hour=9)
        ...
        action_id = self.env.ref('tech.action_tech_event')
        url = "/web#action=" + str(action_id.id) + '&view_type=tech'
        self.start_tour(url, 'test_tech_decline_with_everybody_filter_tour', login='demo')
        attendez = self.env['tech.attendez'].search([('event_id', '=', event.id), ('partner_id', '=', user_demo.partner_id.id)])
        self.assertEqual(attendez.state, 'declined') # Check if the event has been correctly declined
