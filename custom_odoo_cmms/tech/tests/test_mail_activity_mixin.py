...
class TestMailActivityMixin(MailCommon):
    ...
    def setUpClass(cls):
        ...
        cls.activity_type_1 = cls.env['mail.activity.type'].create({
            'name': 'Tech Activity Test Default',
    ...
    def test_activity_tech_event_id(self):
        """Test the computed field "activity_tech_event_id" which is the event of the
        next activity. It must evaluate to False if the next activity is not related to an event"""
        def create_event(name, event_date):
            return self.env['tech.event'].create({
                'name': name,
                'start': datetime.combine(event_date, time(12, 0, 0)),
                'stop': datetime.combine(event_date, time(14, 0, 0)),
            })

        def schedule_meeting_activity(record, date_deadline, tech_event=False):
            meeting = record.activity_schedule('tech.tech_activity_test_default', date_deadline=date_deadline)
            meeting.tech_event_id = tech_event
            return meeting
        ...
        with self.with_user('employee'):
            ...
            self.assertFalse(test_record.activity_tech_event_id, "The next activity does not have a tech event")

            act1.tech_event_id = ev1

            self.assertEqual(test_record.activity_tech_event_id.name, ev1.name, "This should be the tech event of the next activity")
